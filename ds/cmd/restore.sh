...
_make_full_restore() {
    ...
    ds exec drush @qcl sql-drop --yes
    ds exec drush @qcl sql-query \
       --file=/host/$backup/qcl.sql

    # restore application files
    rm -rf var-www/{qcl,downloads}
    cp -a $backup/{qcl,downloads} var-www/

    # restore qcl_dev
    if [[ -f $backup/qcl_dev.sql ]]; then
        ds exec drush @qcl_dev sql-drop --yes
        ds exec drush @qcl_dev sql-query \
           --file=/host/$backup/qcl_dev.sql
